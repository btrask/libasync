...
int MultipartBoundaryFromType(char const *const type, uv_buf_t *const out) {
	assert(out);
	if(!type) return -1;
	char const prefix[] = "multipart/form-data; boundary=";
	size_t const len = sizeof(prefix)-1;
	if(0 != strncmp(type, prefix, len)) return -1;
...
void MultipartFormFree(MultipartFormRef *const formptr) {
	MultipartFormRef form = *formptr; *formptr = NULL;
	if(!form) return;
	form->conn = NULL;
...
int MultipartFormPeek(MultipartFormRef const form, MultipartEvent *const type, uv_buf_t *const buf) {
	...
	for(;;) {
		...
		if(len < 0) return UV_UNKNOWN; // Parse error.
...
int MultipartFormReadHeaderValue(MultipartFormRef const form, char value[], size_t const max) {
	if(!form) return UV_EINVAL;
	uv_buf_t buf[1];
	int rc;
	MultipartEvent type;
...
int MultipartFormReadHeadersStatic(MultipartFormRef const form, uv_buf_t values[], char const *const fields[], size_t const count) {
	...
	int rc;
	MultipartEvent type;
	char field[MULTIPART_FIELD_MAX];
	uv_buf_t value[1];
	size_t i;
...
static int on_header_field(multipart_parser *const parser, char const *const at, size_t const len) {
	MultipartFormRef const form = multipart_parser_get_data(parser);
	form->type = MultipartHeaderField;
	*form->out = uv_buf_init((char *)at, len);
	return -1;
}
static int on_header_value(multipart_parser *const parser, char const *const at, size_t const len) {
	MultipartFormRef const form = multipart_parser_get_data(parser);
